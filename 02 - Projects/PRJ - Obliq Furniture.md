...
> - Excel export now 5 sheets (added "By Finished Product" + "Finished × Raw Detail"). 2 new jsonrpc endpoints (`/raw_detail` + `/finished_detail`). All 3 folders synced (dev / client folder / prod) at v19.0.8.3.1. User confirmed dashboard working post-fix.
> - **Reusable rule for fleet OWL dashboards:** avoid `min-height: 100vh` on client-action root containers — use `height: 100%; overflow-y: auto` instead, otherwise content below the fold becomes unreachable.
