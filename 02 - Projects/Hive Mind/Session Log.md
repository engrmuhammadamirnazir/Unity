...
- **Risk #2 — Staleness check** — `scripts/hive-mind/check_graph_freshness.sh` scans all 5 target graphs, flags any where source files are >7d newer than `graph.json`. Tested: Unity fresh, 4 others missing (expected, needs first `/graphify .`). Integrate into `hive-mind-audit` section 3.
