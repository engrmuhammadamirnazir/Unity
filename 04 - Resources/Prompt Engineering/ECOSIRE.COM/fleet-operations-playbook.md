...
| "Connect to the Odoo API" | @api-integrator | "Odoo" + "API" |
...
```
Use the security-auditor agent to review all authentication endpoints in apps/api/src/auth/
```
...
- Teammate 1: @frontend-engineer (owns apps/web/ — pages, components, styling)
- Teammate 2: @backend-engineer (owns apps/api/ — modules, controllers, services, DTOs)
- Teammate 3: @database-engineer (owns packages/database/ — migrations, entities, queries)
...
Options:
- "Document all API endpoints in apps/api/" → generates OpenAPI docs
