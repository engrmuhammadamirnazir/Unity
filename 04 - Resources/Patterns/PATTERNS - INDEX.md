...
### Sales / Pricing / Competitive
- [[feedback_quote_clients_use_website_prices_not_manifest]] — Canonical price source for client quotes is `D:/ECOSIRE.COM/_update_product_prices.sql` (or live ecosire.com), NOT module `__manifest__.py`. Manifests drift after Wave-style pump upgrades (e.g., 4 modules diverged $150-$250 between manifest and website on 2026-05-05). Drizzle seed `D:/ECOSIRE.COM/packages/db/src/seeds/products.ts` is also stale. After future pumps, also apply `_update_product_prices.sql` to live website DB in same session.
- [[feedback_webkul_comparison_leverage_module_gap]] — When a prospect compares us with Webkul (or any single competitor) and asks for price-matching, research the module-coverage gap first. Webkul typically covers only ~50% of multi-marketplace stacks (Edwin Ardila 2026-05-05: 5 of 10). The gap is the leverage that justifies a competitive bundle floor without dollar-for-dollar matching. Floor IS the offer — below floor = scope reduction, not deeper discount.
- [[feedback_wise_pay_links_are_per_invoice_not_reusable]] — `https://wise.com/pay/r/<id>` URLs are per-payment-request artifacts tied to a specific amount + recipient, NOT a reusable account identifier. Reusing one across clients routes the wrong amount or confuses the payer. Default for new invoices: `WISE_LINK = None` (render Wise option as "recipient: <name> / <email>" — payer initiates send from own Wise app). Bank IBAN `PK73MEZN0022040105909195` IS a stable fleet constant. When canonical `scripts/client_docs/ecosire_invoice_template.py` is rebuilt, `WISE_LINK` MUST be a per-invoice parameter with `None` default. Caught 2026-05-06 carrying Edwin Ardila's $2,000 link onto Bimal Tamrakar's $250 invoice.
- [[feedback_lifetime_support_scoped_to_purchased_version]] — When selling a module to a client for a specific Odoo version, lifetime updates and support apply ONLY to that version, NOT all 17 / 18 / 19. The fact that the same module exists in all 3 odoo{17,18,19}/ trees is for ECOSIRE's backporting workflow — it does NOT mean the customer gets all 3 binaries with one purchase. Cross-version migration is a separate paid line. Edwin Ardila's "free upgrade to v20" is a deliberate same-day-cash-close concession — NOT a fleet template. **Always ask client's Odoo version upfront** as part of invoice intake (same priority as amount + email). Caught 2026-05-06 on Bimal Tamrakar Daraz invoice.

### Client deliverables / Branding
- [[feedback_client_docs_in_client_folder_with_editable_version]] — Every client-facing document goes to `D:\EcosireClients\<ProjectClients|ModulesClients|ProspectiveClients>\<Client>\docs\` (NOT scratch/tmp paths) and ships as a PDF + editable DOCX pair sharing one filename stem. Use `letterhead_full.png` (not split header/footer crops, which lose the side decorations). Page size = A4 to match the 0.707 letterhead aspect. Includes the exact `<wp:anchor behindDoc="1">` XML pattern for python-docx behind-text full-page background (no built-in API).
...
### Odoo / ORM
- [[feedback_odoo19_sale_mrp_smart_button_via_stock_reference]] — Odoo 19's Manufacturing smart button on `sale.order` walks `sale.stock_reference_ids.production_ids` (a NEW v19 M2M model: `stock.reference`), NOT the `move_orig_ids` chain you'd expect from earlier versions. Manual `mrp.production.create()` for an existing locked SO creates its own stock.reference disconnected from the SO's reference — smart button stays at 0. Fix: also `so.write({"stock_reference_ids": [(4, mo_ref.id)]})` or use `_action_launch_stock_rule()`. Bit Obliq twice (2026-05-01 + 2026-05-05). Applies to every v19 client (Oenoteca, Suleman Remittance, Future Vision Waredat, ECOSIRE.IO).
