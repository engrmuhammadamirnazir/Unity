---
type: reference
category: ecosire, clients
tags: [clients, portfolio, ecosire]
updated: 2026-05-01T23:55Z
---

# Ecosire — Client Portfolio

> Active and past clients served by [[Ecosire Private Limited]] and [[Odovation]].

---

## Active Clients — Implementation & Custom Development

### Obliq Furniture Manufacturing LLC
- **Industry**: Contemporary furniture design & manufacturing
- **Location**: Al Quoz Industrial Area 1, Dubai, UAE
- **Platform**: Odoo 19 Enterprise
- **Services**: ERP cleanup, inventory fix, AR/AP reconciliation, Shopify integration, BOM cost breakdown, bank-statement automation
- **Contact**: Ziad Al-Haffar (ziad@homefigures.com); Kamal Nath (Accounts)
- **Fee**: USD 2,500 (confirmed, $1K advance received)
- **Production**: Hetzner 23.88.13.113 (migrated from AWS 2026-04-14; isolated, not shared)
- **Modules**: obliq_automation v19.0.7.0.0; **obliq_statement_auto_import v19.0.1.0.0** (LIVE 2026-04-25 — ADCB/Paymob direct-upload with 5-layer dedup + CC-payoff auto-tag)
- **Key Data**: 585 products, 1,042 BOMs, 619 SOs, AED 1.43M revenue; 1,257 bank-statement lines backfilled with dedup fingerprints
- **Latest bookkeeping state**: 2026-04-25 audit posted 3 correcting JEs (SO 0106/0107/0108) removing AED 353,366 of fake Office Expense; Ziad Current Account, ADCB CC liability, ADCB Studio Obliq bank all reflect true economic position
- **Project**: [[PRJ - Obliq Furniture]]

### Sahara Properties
- **Industry**: Real Estate & Property Management
- **Location**: Dubai, UAE
- **Platform**: Odoo 19 Enterprise (Bitnami)
- **Services**: Custom PMS module, COA setup (94 accounts), analytics, dashboard
- **Production**: https://saharaproperties.ecosire.com (3.232.201.222)
- **Module**: property_management_system v19.0.3.10.0 (projects, blocks, units, deposits, PDCs, utilities, tenant leases, mgmt fees)
- **Data**: 12 projects, 162 tenants, 156 units, 577 PDCs, 70 security deposits, 89 leases
- **Status**: LIVE on test + prod + GitHub
- **Project**: [[PRJ - Sahara Properties]]

### Oenoteca Fine Wines
- **Industry**: Wine distribution and retail
- **Location**: Houston, Texas, USA
- **Platform**: Odoo 19 Enterprise
- **Services**: Bookkeeping, bank reconciliation, financial reporting, landed costs
- **Contact**: [[Niccolo Saltarelli]] (CEO), [[Andrea Graneris]] (COO)
- **Production**: 52.206.177.34 (SHARED with Obliq — CRITICAL)
- **Status**: Q1 IN FINAL REVIEW — PPA $115,774.83 posted, 16 decisions + 77-wine bucketing pending Niccolo
- **Key Data**: 322 invoices, 85 vendor bills, 436 BSLs reconciled, $339K inventory
- **Project**: [[PRJ - Oenoteca Fine Wines]]

### Remittance Management (Salueman Arif)
- **Industry**: Financial Services / Remittance
- **Location**: Pakistan
- **Platform**: Odoo 19 Enterprise (Bitnami)
- **Services**: Custom remittance management module (cross-company consolidation, compliance)
- **Production**: https://remittanceaccounting.ecosire.com (52.28.45.137)
- **Module**: remittance_management v19.0.5.0.2
- **Status**: LIVE on prod + remtest (rate-lock + counterparty validation + bridge UX hotfixes)
- **Key Data**: 351 entries imported from Excel, 3 companies, 2 users, E2E tested (18 flows)

### RedXpider
- **Industry**: Manufacturing / Automated Test Equipment (ATE)
- **Location**: Netherlands
- **Platform**: Odoo 18 Enterprise (Odoo.sh)
- **Services**: Custom module development (3PL EDI integration)
- **Fee**: $3,000 / 4 milestones
- **Codebase**: 3 existing modules (rxp, crm_base_v11, partner_attribution_v1), 138 files total
- **Status**: KICKED OFF — pre-dev PDF with 11 diagrams delivered

### Alvi Dental
- **Industry**: Healthcare / Dental
- **Location**: Pakistan
- **Platform**: Odoo 18 → 19 migration planned (dual-run approach)
- **Services**: ERP implementation, WooCommerce integration
- **Infrastructure**: Hetzner EX44 PROVISIONED at 23.88.13.113 (awaiting Phase A go-ahead)
- **Migration**: Dual-run then cutover, NOT in-place. AWS stays live untouched until Hetzner UAT signoff

### Aligners Pakistan
- **Industry**: Dental/Medical devices
- **Location**: Pakistan
- **Platform**: Odoo (custom development)
- **Services**: Custom store development (patient management, invoicing, scan tracking)

### Future Vision Est. for Custom Clearance (Waredat ERP)
- **Industry**: Customs Clearance + Multi-tenant ERP SaaS platform (Waredat ERP)
- **Location**: Jeddah, Saudi Arabia
- **Platform**: Custom Flask app on Replit (`https://replit.com/@hussamalmohaime/Customs-Flow`) — production at `waredaterp.com`
- **Services**: ZATCA Phase 2 + Phase 3 fix/harden (NOT greenfield — fixing existing implementation)
- **Contact**: Hussam Saleh Almohaimeed (CEO) — `hussam.almohaimeed@cports.sa`
- **Fee**: USD 750 (paid up-front via Meezan Bank PK)
- **Engagement**: `INV-2026-04-FVCC-001`
- **Status**: ZATCA Compliance Sandbox CERTIFIED 9/9 (2026-04-29 evening) for first tenant ABIR ALMANAFETH; deployment to production pending tomorrow (2026-04-30)
- **Sub-tenant cert state**: ABIR ALMANAFETH Establishment Ltd (VAT `300832422400003` / CR `7006925437` / Logistics / Jeddah 22233) — Compliance CSID issued (`requestID=1234567890123`, dispositionMessage=ISSUED), 9/9 official scenarios passed against `https://gw-fatoora.zatca.gov.sa/e-invoicing/developer-portal/compliance/invoices`
- **GitHub**: `https://github.com/hussamalmohaimeed-a11y/waredat-erp` (collaborator access); PR #1 at `/pull/1` open with 5 commits, NOT merged
- **Phase 4 (NEW SCOPE — out of $750 contract)**: Hussam introduced architectural request to move ALL ZATCA settings to admin/developer surface and reduce tenant frontend to "Link with ZATCA" + OTP only. To be quoted separately ($500 lean / $850 standard / $1,500 comprehensive). Don't start without contract.
- **Project**: [[PRJ - Future Vision Waredat]] (TBD)

---

## Active Clients — Module Delivery (App Store Customers)

### Tariq Almaimani (Smart Beauty) — WITHDRAWN 2026-05-13
- **Industry**: E-commerce / Beauty
- **Location**: Saudi Arabia
- **Platform**: Odoo.sh (`tmaimani-erpdev` — staging)
- **Module**: Trendyol Store Management v2.1.0 — previously deployed 2026-04-05
- **Status**: **WITHDRAWN 2026-05-13** — never converted to payment after the 2026-04-05 eval deployment. Code removed from `tmaimani/erpdev` branch `st1` (commit `0c4e1fe`); his own `account_report/` untouched. Both license keys deactivated: comp `ECO-3EKD-9VRB-WDEE` revoked earlier today on the ECOSIRE.COM licensing app, activation key `ECO-VGMX-TUB7-BQVA` deactivated on the license server.
- **Data at peak**: 80 orders, 78 customers synced on company 3 (Smart Beauty / جمالي الذكي), Arabic translated (1,206/1,211 strings)
- **Re-engagement**: payment lands → re-push from `D:/Development/EcosireSolutions/Trendyol-Store-Management/`, issue fresh license key, redeploy on company 3 / warehouse 2 (البحر). Playbook: `~/.claude/projects/D--Development/memory/feedback_prospective_deploy_no_pay_withdraw_code.md`.

### Quicken Accounting (Soovah)
- **Industry**: Accounting / E-commerce
- **Domain**: odoo.soovah.com (5.78.207.108)
- **Platform**: Docker Odoo 19
- **Module**: Wayfair Store Management **v19.0.3.12.1** (deployed 2026-04-29 — backlog stranding fix + date_order pin)
- **Status**: LIVE — EDI 820 payout import, settlement CSV wizard, reconciliation, payment registration, backlog-recovery cron self-healing
- **Data**: 5,567 wayfair_order mappings; CS=4,515 / CA=235 / DS=0
- **2026-04-29 incident**: 1,470 missing CS/CA POs Feb 19 – Apr 2 recovered to prod (root cause = v3.11.0 backlog stranding bug, fixed in v3.12.0 + cosmetic date_order side-effect fixed in v3.12.1). Invoices: Feb 295 / Mar 1,114 / Apr 62 all `posted` with correct invoice_date.
- **Additional**: SAP B1 ↔ Odoo integration proposal sent ($1,250, 2 weeks)

### Gaspar Muhlmann
- **Industry**: Odoo Partner / E-commerce
- **Location**: Argentina
- **Module**: MercadoLibre Store Management v1.2.0
- **Status**: Channel partner with multiple clients, Phase 1 features delivered
- **Note**: Published + deployed to both demo servers after deep fix session (20+ fixes)

### Rebecca Lee
- **Industry**: E-commerce
- **Location**: China
- **Module**: Amazon Store Management (Odoo 18)
- **Status**: $1K proposal sent (2026-03-23)

---

## Prospective Clients

| Client | Industry | Location | Proposal | Status |
|--------|----------|----------|----------|--------|
| Ximplifika | Import/Trading | Mexico | $3,500 ERP | Sent 2026-03-26 |
| Texas Closeout | 3PL WMS | Houston, TX | $1,999 (via Mastero) | Sent 2026-03-26 |
| Airport Taxi Hub | Airport transfers marketplace | Delaware/UK | $4,000 ERP | Sent 2026-04-03 (Quicken referral) |
| Clube de Literatura Clássica (Editora Literatura Classica Ltda., CNPJ 34.071.390/0001-02) | B2C subscription book club, 15k+ assinantes, ~37 employees | Dois Irmãos, RS, Brazil | Odoo 19 + l10n_br pitch (subscriptions + NF-e/ICMS/PIS-COFINS + monthly box fulfillment) — 4 prep questions + meeting slot proposed | Inbound 2026-04-29; reply sent in PT-BR; meeting proposed 05/05 or 06/05 14h00 BRT — awaiting confirmation. Contact: Lisandra Spohr (Financeiro) financeiro@literaturaclassica.com.br / +55 51 98064-4101 |
| OTRA Technology Co., Ltd. (又一科技; Reg #83038170) | Official Odoo Partner Taiwan — partner-eval, not end-user | Taipei, Taiwan | ECOSIRE Shopee Store Management module — $499 App Store license, 13 regions, partner-volume conversation opened | Inbound 2026-04-29 from Odoo App Store; reply sent (demo URL + docs + pricing + 3 Taipei call slots Mon/Tue/Wed May 4-6); demo creds follow-up + ecosire.com price drift fix ($249→$499) still open. Contact: Iris Chau 周雨婷 irischau@otra-technology.com / +886-2-7729-6651 #11 |
| Brad & Brad Fine Dining (Cote Rotie PK) | Fine-dining restaurant | Pakistan | INV-2026-04-BB-001 PKR 60K (PKR 50K POS setup + PKR 10K May hosting) sent 2026-04-29; 2026-05-12 ERP Analysis Report adds 3 tiers — (A) POS only PKR 50K+10K/mo, (B) Community ERP $350+$35/mo, (C) Enterprise Custom self-hosted $500+$35/mo + $10.90/user/mo Odoo SA (recommended) | Demo live at restaurant.demo.ecosire.com (admin/admin) from 2026-05-01; live demo meeting 2026-05-02; ERP Analysis Report sent 2026-05-12 awaiting tier decision; PKR 60K payment on POS invoice still outstanding. Existing ERPNext to be migrated (separately quoted). Contact: Mr. Aamir Habib (Finance Manager) aamir.habib@coterotiepk.com |

---

## Inactive / Past Clients

| Client | Industry | Notes |
|--------|----------|-------|
| Mastero (mastero.net) | Technology | Server customization, MuK theme, accounting modules |
| PSL Premium Sports Lighting | Sports equipment | Custom ERP, Gmail integration |
| Sir Sohail Baig / UsTelekom | Telecom/VOIP | Odoo 16-17, portal.jixi.live |
| Dynasty Tahiti | Business ops | Tahiti, ERP setup |
| DTS Connector | Integration | Custom connector project |
| Dr Muneera | Healthcare | ERP implementation |
| BSD TECH | Technology | Odoo hosting (40.172.205.236) |
| Tigo | Telecom | Odoo ERP management |
| ERPSAP.IN (Shree Group) | Manufacturing | India, erpsap.in |
| Presto Pizza Company | Food & Restaurant | erp.prestopizzacompany.com |

---

## Revenue Sources

### Recurring Monthly
| Client | Amount | Method |
|--------|--------|--------|
| Oenoteca Fine Wines | $150 USD/mo | Wise transfer |
| Alvi Dental | PKR 25,000/mo | Meezan Bank |
| Obliq Furniture | $35 USD/mo (ECOSIRE Managed Hosting) | **Meezan Bank** (USD-billed exception — pays via Meezan, NOT Wise) |
| Sahara Properties | TBD USD/mo | **Meezan Bank** (USD-billed exception — pays via Meezan, NOT Wise) |

**Invoice rules (canonical, established 2026-05-01 — applies to ALL Ecosire client invoices regardless of which workspace generates them):**
1. **Due date** on monthly recurring invoices = `"Due on receipt"` — never a +14/+15-day calendar date.
2. **Hosting line items** = `"ECOSIRE Managed Hosting - {Month} {Year}"` — never "Amazon AWS Hosting", "AWS Hosting", or "Hosting Charges". Reinforces managed-service positioning.
3. **Payment rail** by default: USD → Wise (Amir's Wise email). PKR → Meezan (`info@ecosire.com`). **Exceptions:** Obliq + Sahara are USD-billed but pay via **Meezan only** — do NOT list Wise on their invoices. Single rail per invoice; no dual listing.
4. Canonical generator at `D:/Development/scripts/client_docs/ecosire_invoice_template.py` — never re-implement. Reference invoice: Obliq INV-2026-05-003 (Apr+May 2026 hosting, $70 USD/Meezan).

### Module Sales (App Store)
- 201 modules at $249-$599 each
- 17 currently published on Odoo App Store
- Target: all 201 published by mid-2026

### Custom Development
- Obliq: $2,500 (one-time)
- Sahara: Custom PMS (ongoing)
- Remittance: Custom module (ongoing)
- RedXpider: $3,000 / 4 milestones

---

## Key Stats

- 500+ businesses served globally
- 2,500+ Odoo implementations
- 50+ countries
- 33+ industries
- 6 service lines (Odoo, Shopify, OpenClaw AI, GoHighLevel, Power BI, Accounting)

---

## Industries Served

- Wine & beverage distribution
- Healthcare / Dental / Medical devices
- Real estate & Property management
- Financial services / Remittance
- Manufacturing / ATE
- Furniture design & manufacturing
- Retail & eCommerce / Beauty
- Telecommunications / VOIP
- Food & Restaurant
- Technology services
- Sports equipment
- Airport transfers / Marketplace

---

## Client Infrastructure

### SSH Keys
- AWS PPK files at `C:\Users\Amir Nazir\Dropbox\Keys\AWS\`
- Ecosire Main ed25519 at `~/.ssh/ecosire_main` (for Hetzner EX44)

### Client Data
- Project clients: `D:\EcosireClients\ProjectClients\{Name}\` (was `ActiveClients/`, renamed 2026-05-09)
- Modules clients: `D:\EcosireClients\ModulesClients\{Name}\` (NEW 2026-05-09 — off-the-shelf module buyers)
- Custom module repos: github.com/engrmuhammadamirnazir (PRIVATE)
- Documents/Proposals: `D:\Development\Quotations\`

---

## Related

- [[Ecosire Private Limited]] — Company
- [[Ecosire - Production Servers]] — Server inventory
- [[MOC - Projects]] — Project index
- [[MOC - People]] — Client contacts
